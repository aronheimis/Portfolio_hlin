      ...
      {photos.map((photo, i) => (
        ...
        >
          <div className="relative overflow-hidden bg-parchment-100 group">
            <Image
              src={urlFor(photo.image).width(900).auto('format').url()}
              alt={photo.alt}
              width={900}
              height={0}
            ...
            {photo.caption && (
              <div className="absolute inset-0 flex items-end bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
